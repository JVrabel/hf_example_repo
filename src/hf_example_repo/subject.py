...
class TokenIdStreamer(BaseStreamer):
    ...
    def put(self, value: torch.Tensor):
        ...
        assert isinstance(value, torch.Tensor), "TokenIdStreamer only expects streaming torch.Tensors"
...
class Subject:
    ...
    def __init__(
        ...
        self.w_ins = {
            layer: fetch_attr(self.model, config.w_in_module_template.format(layer=layer)) for layer in range(self.L)
        }
        self.w_gates = {
            layer: fetch_attr(self.model, config.w_gate_module_template.format(layer=layer)) for layer in range(self.L)
        }
        self.w_outs = {
            layer: fetch_attr(self.model, config.w_out_module_template.format(layer=layer)) for layer in range(self.L)
        }
        self.layers = {
            layer: fetch_attr(self.model, config.layer_module_template.format(layer=layer)) for layer in range(self.L)
        }
        self.mlps = {
            layer: fetch_attr(self.model, config.mlp_module_template.format(layer=layer)) for layer in range(self.L)
        }
        self.attns = {
            layer: fetch_attr(self.model, config.attn_module_template.format(layer=layer)) for layer in range(self.L)
        }
        self.attn_vs = {
            layer: fetch_attr(self.model, config.v_proj_module_template.format(layer=layer)) for layer in range(self.L)
        }
        self.attn_os = {
            layer: fetch_attr(self.model, config.o_proj_module_template.format(layer=layer)) for layer in range(self.L)
        }
        self.input_norms = {
    ...
    def tokenize_single(self, text: str) -> int:
        ...
        assert len(toks) == 1, f"Expected 1 token, got {len(toks)}: {[self.decode(t) for t in toks]}"
        return toks[0]
    ...
    def apply_logit_lens(
        ...
        with torch.no_grad():
            ...
            if normalize_logits:
                logits_X_V -= torch.einsum("d,...d->...", unembed_orig_VD.mean(dim=0), dirs_X_D).unsqueeze(-1)
...
class Llama3Config(LMConfig):
    ...
    layernorm_fn: Callable[[torch.Tensor, torch.Tensor, torch.Tensor, float], torch.Tensor] = _llama3_layernorm_fn
...
def make_subject(config: LMConfig, dispatch: bool = True, disable_flash_attention: bool = True):
    return Subject(
        config,
...
class GPT2Config(LMConfig):
    unembed_module_str: str = "lm_head"
    unembed_norm_module_str: str = "transformer.ln_f"
    w_in_module_template: str = "transformer.h.{layer}.mlp.c_fc"
    w_gate_module_template: str = ""  # Not applicable for GPT-2
    w_out_module_template: str = "transformer.h.{layer}.mlp.c_proj"
    layer_module_template: str = "transformer.h.{layer}"
    mlp_module_template: str = "transformer.h.{layer}.mlp"
    attn_module_template: str = "transformer.h.{layer}.attn"
    v_proj_module_template: str = "transformer.h.{layer}.attn.c_attn"
    o_proj_module_template: str = "transformer.h.{layer}.attn.c_proj"
    input_norm_module_template: str = "transformer.h.{layer}.ln_1"

    I_name: str = "n_embd"  # Intermediate size (using n_embd for GPT-2)
    D_name: str = "n_embd"  # Residual stream size
    V_name: str = "vocab_size"  # Vocab size
    L_name: str = "n_layer"  # Num layers
...
# Update the existing get_subject_config function to include GPT-2
